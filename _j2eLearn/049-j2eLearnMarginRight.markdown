...
```
marginRight: length | percentage
```

| 속 성 | 형 식|
|---|---|
| length \| percentage | marginRight:"Xpx" |
...
j2e.addRole({name:"role_1", role:[{share: 100, marginRight:"length | percentage"}]});
...
* #### 예제소스
<pre class="prettyprint linenums:1">
j2e.addRole({name:"role_1", role:[{share: "100", marginRight:"50px"}]});
...
<pre class="prettyprint linenums:1">
j2e(elements).animate({role:[{duration: 1, marginRight:"length | percentage"}]});
</pre>
...
$(document).ready(function(){
  ...
  $("#trasitionButton").click(function(){
    ...
    j2e("#trasitionButton").animate({role:[{duration: 1, marginRight:value}]});
  }
});
</pre>
