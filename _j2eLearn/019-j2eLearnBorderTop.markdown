...
```
borderTop: borderWidth border-style [borderColor]
```
...
j2e.addRole({name:"role_1", role:[{share: 100, borderTop:"borderWidth border-style [borderColor]"}]});
...
* #### 예제소스
<pre class="prettyprint linenums:1">
j2e.addRole({name:"role_1", role:[{share: "100", borderTop:"15px solid red"}]});
...
<pre class="prettyprint linenums:1">
j2e(elements).animate({role:[{duration: 1, borderTop:"borderWidth border-style [borderColor]"}]});
</pre>
...
$(document).ready(function(){
  ...
  $("#trasitionButton").click(function(){
    ...
    j2e("#trasitionButton").animate({role:[{duration: 1, borderTop:value}]});
});
</pre>
