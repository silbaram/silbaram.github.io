...
lastmod: 2016-07-28 16:59:12 +0900
---
...
```
borderTopLeftRadius: length [length|percentage] | percentage [length|percentage]
```

| 속 성 | 형 식|
|---|---|
| length [length \| percentage] | borderTopLeftRadius: "Xpx [Ypx \| Y%]" |
| percentage [length\|percentage] | borderTopLeftRadius: "X% [Ypx \| Y%]" |
...
j2e.addRole({name:"role_1", role:[{share: 100, borderTopLeftRadius:"length [length|percentage] | percentage [length|percentage]"}]});
...
* #### 예제소스
<pre class="prettyprint linenums:1">
j2e.addRole({name:"role_1", role:[{share: "100", borderTopLeftRadius:"50"}]});
...
<pre class="prettyprint linenums:1">
j2e(elements).animate({role:[{duration: 1, borderTopLeftRadius:"length [length|percentage] | percentage [length|percentage]"}]});
</pre>
...
$(document).ready(function(){
  ...
  $("#trasitionButton").click(function(){
    ...
    j2e("#trasitionButton").animate({role:[{duration: 1, borderTopLeftRadius:value}]});
  }
});
</pre>
