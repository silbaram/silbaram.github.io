...
j2e.addRole({name:"role_1", role:[{share: 100, borderRightWidth:"length"}]});
...
* #### 예제소스
<pre class="prettyprint linenums:1">
j2e.addRole({name:"role_1", role:[{share: "100", borderRightWidth:"50"}]});
...
<pre class="prettyprint linenums:1">
j2e(elements).animate({role:[{duration: 1, borderRightWidth:"length"}]});
</pre>
...
$(document).ready(function(){
  ...
  $("#trasitionButton").click(function(){
    ...
    j2e("#trasitionButton").animate({role:[{duration: 1, borderRightWidth:value}]});
  }
});
</pre>
