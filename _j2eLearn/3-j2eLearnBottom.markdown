...
> # **Bottom**
> 애니메이션 효과를 주기 위해 선택된 element를  현 지점을 기준으로 +면 위으로 -면 아래쪽으로 이동 시킵니다.
...
```.javascript
j2e.addRole({name:"role_1", role:[{share: 0, bottom:"시작점"}, {share: 100, bottom:"종착점"}]});
...
```.javascript
j2e(elements).animate({role:[{duration: 1, bottom:”종착점”}]});
```
* j2e(elements) 부분에서 효과를 주고싶은 element선택합니다.  
  방법은 jQuery와 같습니다.  
